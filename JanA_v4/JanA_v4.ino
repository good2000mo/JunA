...
void PID(double restAngle, double offset, double turning) {
  ...
  integratedError = constrain(integratedError, -20.0, 20.0); // Limit the integrated error
...
void steer(Command command) {
  ...
  if (command == joystick) {
    ...
    if (sppData1 > 0) {
      turningOffset = scale(sppData1,0,1,0,15);
      steerRight = true;
    } else if (sppData1 < 0) {
      turningOffset = scale(sppData1,0,-1,0,15);
      steerLeft = true;
...
void leftEncoder() { 
  if ((bool)(PIND & _BV(PIND2)) == (bool)(PINC & _BV(PINC0)))
    leftCounter++;
  else
    leftCounter--;
}
void rightEncoder() {
  if ((bool)(PIND & _BV(PIND3)) == (bool)(PINC & _BV(PINC1)))
    rightCounter--;
  else
    rightCounter++;
}
